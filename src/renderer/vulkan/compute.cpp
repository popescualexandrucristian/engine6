...
static bool user_update(acp_vulkan::renderer_context* context, size_t current_frame, VkRenderingAttachmentInfo color_attachment, VkRenderingAttachmentInfo depth_attachment, double)
{
	...
	vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, user->graphics_program->pipeline_layout, 0, 1, &user->descriptor_set_resources, 0, nullptr);
...
static bool user_init(acp_vulkan::renderer_context* context)
{
	...
	vertex_shader_input_attributes.offsets = { 
		offsetof(compute_user_data::vertex, position), 
		offsetof(compute_user_data::vertex, color),
		offsetof(compute_user_data::vertex, uv) };
	vertex_shader_input_attributes.locations = { 0, 1, 2 };
	vertex_shader_input_attributes.stride = sizeof(compute_user_data::vertex);
	...
	user->compute_shader = acp_vulkan::shader_init(context->logical_device, context->host_allocator, "./shaders/example.comp.spv");
	...
	compute_user_data::vertex verts_data[4] = {
	...
	user->vertex_data = acp_vulkan::upload_data(context, verts_data, 4, sizeof(compute_user_data::vertex), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
	...
	descriptor_info.pSetLayouts = &user->graphics_program->descriptor_layouts[0].first;
